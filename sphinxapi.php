...
class SphinxClient
{
	...
	{
		// redis instance 
		$this->_redis_scheme	= "tcp";
		$this->_redis_host	= "localhost";
		$this->_redis_port	= 6379;
                $this->_redis_prefix       = "sphinx_search_";
		$this->_redis_ttl	= 1800;
		$this->_redis_conn	= false;
	...
	{
		$this->_redis_conn = new Predis\Client([
		    'scheme' => $this->_redis_scheme,
		    'host'   => $this->_redis_host,
		    'port'   => $this->_redis_port,
		]);
	}

	// set redis host name (string) and port (integer)
	function SetRedisServer( $scheme, $host, $port )
	{
		$this->_redis_scheme = $scheme;
		$this->_redis_host = $host;
		$this->_redis_port = $port;
	}

	function SetRedisServerTTL($ttl)
	{
		$this->_redis_ttl = $ttl;
	}
	...
	function RunQueries ()
	{
		...
		$redisResult = $this->getQueryData($nreqs,$req);
		if($redisResult)
		{
			return $redisResult;
		}
		...
		$result = $this->_ParseSearchResponse ( $response, $nreqs );
		$this->setQueryData($nreqs,$req,$result);
		return $result;
	}

	function getQueryData($nreqs,$req)
	{
		return $this->_redis_conn->get($this->_redis_prefix."_".$nreqs."_".$req);
	}
	function setQueryData($nreqs,$req,$result)
	{
		$client->set($this->_redis_prefix."_".$nreqs."_".$req, $result,null,$this->_redis_ttl,null);
	}
